  ...
  useEffect(() => {
    ...
    for (let i = 1; i < 100; i++) {
      ...
      data.push({
        ...
        salary: "######",
        tmstatus: (
          <button style={{ width: "130px" }} class="active">
            Active
          </button>
        ),
        accntactivation: (
          <label class="switch">
            <input data-onstyle="success" type="checkbox" />
            <span class="slider round"></span>
          </label>
        ),
        lastwrkdate: k,
    ...
    filtersupervisor = [...new Set(filtersupervisor.map(JSON.stringify))].map(
      JSON.parse
    );
    filtersrsupervisor = [
      ...new Set(filtersrsupervisor.map(JSON.stringify)),
    ].map(JSON.parse);
    setsearch(data);
    setteamList(data);
    const columns = [
